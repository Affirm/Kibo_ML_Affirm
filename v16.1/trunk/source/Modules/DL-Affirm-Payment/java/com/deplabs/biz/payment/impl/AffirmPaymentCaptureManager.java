...
import com.marketlive.biz.payment.*;
...
import org.marketlive.entity.cart.order.*;
...
public class AffirmPaymentCaptureManager extends PaymentCaptureManager implements IAffirmPaymentCaptureManager {
	...
	private static Logger logger = LoggerFactory.getLogger(PaymentCaptureManager.class);
	...
	public PaymentCaptureResult capturePayment(IOrderShipment orderShipment, boolean isCCOMsettlement, boolean isJobProcessing, boolean isRealTimeCapture, String affirmRequestId) {
		...
		if (PaymentUtil.isPaymentThroughCreditCardOrCardTokenOrCardPresent(order.getPayments())) {
			...
			if (amountToCharge != null && !shipmentPaidByCash) {
				...
				if (formattedAmount != null && amountToCharge != null && amountToCharge.compareTo(new Amount()) == 1) {
					...
					if (authorization != null) {
						...
						creditCardRequest.populateRequestPayments(order.getPayments());
						...
						if (creditCardResponse.isSuccess()) {
							...
							//IPaymentGateway paymentGateway = paymentTransactionManager.getPaymentGatewayByCreditCardRequest(type);
							IPaymentGateway paymentGateway = paymentTransactionManager.getPaymentGatewayByPaymentRequest(creditCardRequest);
							String paymentechPaymentGatewayName = configurationManager.getAsString(PAYMENTECH_PAYMENT_GATEWAY_NAME);
						...
						if (PaymentUtil.isPaymentThroughCreditCard(order) || PaymentUtil.isPaymentThroughCardToken(order)) {
							CreditCardRequest creditCardRequest = new CreditCardRequest();
							creditCardRequest.populateRequestPayments(order.getPayments());
			...
			if (amountToCharge != null) {
				...
				if (formattedAmount != null && amountToCharge != null && amountToCharge.compareTo(new Amount()) == 1) {
					...
					if (authorization != null) {
						...
						amazonRequest.setSellerId(configurationManager.getAsString(BIZ_PAYMENT_AMAZONPAYMENT_SELLER_ID));
						...
						String authorizationID = authorization.getCustomFields() != null ? authorization.getCustomFields().get(AmazonPaymentConstants.AMAZON_AUTHORIZATION_ID) : STR_EMPTY;
						...
						if (amazonResponse.getResponseCode()!= null && (amazonResponse.getResponseCode().equals(AmazonPaymentConstants.CAPTURE_RESPONSE_CODE_COMPLETED) || amazonResponse.getResponseCode().equals(AmazonPaymentConstants.CAPTURE_RESPONSE_CODE_PENDING))) {
							authorization.setAuthorizationUsed(true);
							authorizationDAO.update(authorization);
						}
					}
			...
			if (captureTransactionRequired) {
				if (amazonResponse.getResponseCode()!= null && (amazonResponse.getResponseCode().equals(AmazonPaymentConstants.CAPTURE_RESPONSE_CODE_COMPLETED) || amazonResponse.getResponseCode().equals(AmazonPaymentConstants.CAPTURE_RESPONSE_CODE_PENDING))) {
					...
					if(order.getStatus().equalsIgnoreCase(IOrderManager.OrderStatus.COMPLETE.getName())) {
						...
						amazonRequestForClose.setSellerId(configurationManager.getAsString(BIZ_PAYMENT_AMAZONPAYMENT_SELLER_ID));
