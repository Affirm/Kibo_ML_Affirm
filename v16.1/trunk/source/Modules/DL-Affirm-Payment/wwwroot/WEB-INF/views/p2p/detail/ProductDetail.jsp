...
<span itemscope itemtype="http://schema.org/Product">
     ...
     <form:form method="post" modelAttribute="addToBasketForm" id="mainForm"
        ...
        <div class="ml-product-detail">
            <div class="ml-product-detail-wrapper">
                ...
                <c:choose>
					...
					<%-- Affirm Monthly Payment Price --%>
					<c:if test="${MonthlyPaymentPriceModel.attributeMap['isAffirmMonthlyPaymentProductEnabled'] && !borderFreeEnabledAndNotUSSelected}">
                        ...
                        <c:if test="${ProductDetailModel.hasSkus}" >
                            <div class="ml-product-btn-primary">
                                <mlabtest:element uniqueIdentifier="detail_addToBasketButton" description="Add To Basket Button (Page Level)" uriSpecific="false">
                                    <input type="submit" name="addToBasket" class="ml-button-add-basket" value="<fmt:message key='btn.altTxt.addToBasket' />">
                                </mlabtest:element>
                            </div>
                        </c:if>
        ...
        <c:if test="${ProductDetailModel.pickupInStoreEnabled}">
            <input type="hidden" name="latitude" id="latitude">
            <input type="hidden" name="longitude" id="longitude">
            <mlperf:script type="text/javascript" src='${ProductDetailModel.googleJavaScriptUrl}'></mlperf:script>
            <mlperf:script defer="true">MarketLive.P2P.PickupInStoreAddToBasket.initializePickupInStoreAddToBasket('${mlnav:escapeJavaScript(noQtyMsg)}', '${mlnav:escapeJavaScript(noOptionsMsg)}');</mlperf:script>
        </c:if>
...
<div id="productData" style="display:none;" data-product-id="${ProductDetailModel.categoryItem.pk}" data-product-name="${fn:escapeXml(ProductDetailModel.categoryItem.name)}" />
