...
<tiles:importAttribute name="hideSwatches" ignore="true" />
...
<tiles:importAttribute name="optionIDs" ignore="true" />
<tiles:importAttribute name="urlOverride" ignore="true" />
<tiles:importAttribute name="suppressLink" ignore="true" />
<tiles:importAttribute name="optionSelected" ignore="true" />
...
<c:set var="isProductSkuSet" value="${(categoryItem.pk.entityType == 'productSkuSet')}" />
...
<c:if test="${empty showSwatchesForSkuBasedItems}" >
    <c:set var="showSwatchesForSkuBasedItems" value="${ThumbnailModel.showSwatchesForSkuBasedItems}" scope="page"  />
</c:if>
<c:if test="${!showSwatchesForSkuBasedItems && isProductSkuSet}">
    <c:set var="hideSwatches" value="true" scope="page"  />
</c:if>
<c:if test="${empty appendOptionNameToSkuBasedItems}" >
    <c:set var="appendOptionNameToSkuBasedItems" value="${ThumbnailModel.appendOptionNameToSkuBasedItems}" scope="page"  />
</c:if>
...
<c:if test="${not empty urlOverride}">
    <c:set var="categoryItemURL" value="${urlOverride}" />
</c:if>

<%-- Default Swatch and Facet Selection Suppot --%>
<%@ include file="/WEB-INF/views/p2p/ThumbnailSwatchesSupport.jsp" %>
<div class="ml-grid-item-image ml-thumb-item <c:if test='${onselect}'> carousel-selected</c:if>">
    <div class="ml-thumb-image-container">
    <c:choose>
        <c:when test="${useMiniThumb}">
            <div class="ml-thumb-image-mini"><c:if test="${!suppressLink}"><a href="${mlurl:replaceAllUriAmpersands(categoryItemURL)}" data-item-name="<c:out value="${categoryItem.name}" escapeXml="false" />"></c:if><mlimg:prodimg src="${swatchImgSrc}" imageRoot="${catalogImages.imagePath}" productID="${categoryItem.pk.asString}" imageType="mini" entityType="${categoryItem.entityType}" deferLoading="${deferLoadingThumbnail}" deferredPlaceholder="${siteImages.imagePath}global/globalgraphics/deferred_placeholder.gif" otherAttrs='alt="${fn:escapeXml(categoryItem.name)}" ' useDefaultSwatch="true" option="${optionSelected}"/><c:if test="${!suppressLink}"></a></c:if></div>
        </c:when>
        <c:otherwise>
            <div class="ml-thumb-image"><c:if test="${!suppressLink}"><a href="${mlurl:replaceAllUriAmpersands(categoryItemURL)}" data-item-name="<c:out value="${categoryItem.name}" escapeXml="false" />"></c:if><mlimg:prodimg src="${swatchImgSrc}" imageRoot="${catalogImages.imagePath}" productID="${categoryItem.pk.asString}" imageType="thumb" entityType="${categoryItem.entityType}" deferLoading="${deferLoadingThumbnail}" deferredPlaceholder="${siteImages.imagePath}global/globalgraphics/deferred_placeholder.gif" otherAttrs='alt="${fn:escapeXml(categoryItem.name)}" ' useDefaultSwatch="true" option="${optionSelected}"/><c:if test="${!suppressLink}"></a></c:if></div>
        </c:otherwise>
    </c:choose>
    </div>
    <c:if test="${showSwatches and not hideSwatches}">
    ...
    <div class="ml-grid-item-info ml-thumb-info">
        ...
        <c:if test="${showName}">
            <div class="ml-thumb-name ${carouselSelected} ${carouselWordwrap}">
                <a href="<c:url value='${mlurl:replaceAllUriAmpersands(categoryItemURL)}' />" data-item-name="<c:out value="${categoryItem.name}" escapeXml="false" />">
                    <c:choose>
                        ...
                        <c:otherwise>
                            ...
                            <c:if test="${isProductSkuSet && !empty swatchName && appendOptionNameToSkuBasedItems}" >
                                <c:out value="${swatchName}"/>
                            </c:if>
                        </c:otherwise>
                    </c:choose>
                </a>
        ...
        <c:if test="${showShortDescription}">
            ...
            <div class="ml-thumb-desc-short ${carouselSelected} ${carouselWordwrap}">
                 <a href="<c:url value='${mlurl:replaceAllUriAmpersands(categoryItemURL)}' />" data-item-name="<c:out value="${categoryItem.name}" escapeXml="true" />">
                      <c:choose>
