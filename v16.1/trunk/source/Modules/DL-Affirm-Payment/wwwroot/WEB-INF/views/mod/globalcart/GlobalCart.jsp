...
<c:set var="formatPriceData" value="${BorderFreeDataModel.formatPriceDataBasket}" scope="request"/>
...
<c:set var="frontLoadCoefficient" value="${PricingModel.frontLoadCoefficient}" scope="request"/>
<c:set var="lcpMerchandiseTotal" value="0.00" scope="request"/>
<c:choose>
    <c:when test ="${borderFreeEnable && !selectedCountryIsUS && !empty basketTotalMap}">
        ...
        <c:set var="specialOrderDiscount" value="${basketTotalMap['specialOrderDiscount']}" />		
    </c:when>
    <c:otherwise>
        <c:set var="merchandiseTotal" value="${GlobalCartModel.basket.merchandiseTotal}" />
        <c:set var="basketSubTotal" value="${GlobalCartModel.basketSubTotal}" />
...
<c:set var="totalSummary">
    <c:if test="${GlobalCartModel.showCartSubtotal}">
        <fmt:message key="msg.globalCart.totalSummary">
            <fmt:param>
                <mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${GlobalCartModel.basketSubTotal}" />
            </fmt:param>
        </fmt:message>
    </c:if>
</c:set>
...
<div id="globalBasket" class="popDownWrapper globalCartWrapper ml-globalcart-container">
    ...
    <div class="popDownLayer globalCartLayer popover bottom">
        ...
        <tiles:insertAttribute name="discountThresholdMessaging">
            <tiles:putAttribute name="filter" value="BASKET_INCLUDE"/>
...
<c:set var="isRestrictedProduct" scope="request">false</c:set>
<c:choose>
    <c:when test="${borderFreeEnable}">
		<c:forEach var="basketItem" varStatus="status" items="${basketItems}">	
			<ct:call object="${PricingModel}" method="isProductRestricted" param1="${basketItem.product.code}" return="restrictedProduct"/>
		    <c:if test="${restrictedProduct == 'true'}">
				<c:set var="isRestrictedProduct" value="true"  scope="request"></c:set>
			</c:if>	
		</c:forEach>
     </c:when>
     <c:otherwise>
         <c:set var="isRestrictedProduct" scope="request">false</c:set>
                ...
                <c:forEach var="basketItem" varStatus="status"
                                        ...
                                        <c:when test="${regularPrice < skuRegularPrice}">
                                            <c:set var="ProductPrice">
                                                <div class=cartmenupriceitem>
                                                    <div>
														<c:choose>
															 <c:when test="${isRestrictedProduct == 'true'}">
																<mlamt:formatAmount formatPriceData ="${formatPriceData}"  frontLoadCoefficient="${frontLoadCoefficient}" value="${regularPrice}" />
															 </c:when>
															 <c:otherwise>
                                                        <mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${regularPrice}" />
															 </c:otherwise>
														</c:choose>                                                        
                                                    </div>
                                                    <span class=pricewas><mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${skuRegularPrice}" /></span>
                                                </div>
                                            </c:set>
                                        </c:when>
                                        <c:otherwise>
                    ...
                    <c:set var="optionIDs" value=""/>
                    <c:forEach var="option" items="${basketItem.cartItemType.optionsSorted}" varStatus="optionStatus">
                        <c:set var="optionIDs">${optionIDs}${option.pk.asString}${!optionStatus.last ? ',' : ''}</c:set>
                    </c:forEach>
                    <c:set var="thumbnailHTML">
                        <tiles:insertAttribute name="productthumbnail">
                            <tiles:putAttribute name="useMiniThumb" value="${GlobalCartModel.useMiniThumbnail}" />
                            <tiles:putAttribute name="showName" value="false" />
                            <tiles:putAttribute name="showShortDescription" value="false" />
                            <tiles:putAttribute name="showPrices" value="false" />
                            <tiles:putAttribute name="showRating" value="false"/>
                            <tiles:putAttribute name="showFBLike" value="false"/>
                            <tiles:putAttribute name="showSwatches" value="true"/>
                            <tiles:putAttribute name="hideSwatches" value="true"/>
                            <tiles:putAttribute name="showLongDescriptionForDirectoryListView" value="false"/>
                            <tiles:putAttribute name="product" value="${basketItem.product}" />
                            <tiles:putAttribute name="categoryItem" value="${basketItem.product}" />
                            <tiles:putAttribute name="optionIDs" value="${optionIDs}" />
                            <tiles:putAttribute name="urlOverride" value="${ProductThumbnailLink}" />
                            <tiles:putAttribute name="suppressLink" value="${empty ProductThumbnailLink? true:false}" />
                        </tiles:insertAttribute>
                    </c:set>

                    <c:if test="${not empty selectedOptionURL && not empty ProductLink}"><c:set var="ProductLink" value="${selectedOptionURL}" /></c:if>

                    <div class="${globalCartItemInfoCSS}">
                        <div class="nameQtyAndImage">
                            <div class="itemImage">${thumbnailHTML}</div>
...
<!--BorderFree - item discount -->
                            <c:choose>
                                <c:when test="${itemTotalDiscount.toBigDecimal() > 0}">
                                    <div class="discount">
                                        -<mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${itemTotalDiscount}" />
                                    </div>
                                </c:when>
                                <c:otherwise>
                            ...
                            <c:choose>
                                ...
                                <c:otherwise>
                                    <div class="price">
										...
										<c:choose>
											 <c:when test="${isRestrictedProduct == 'true'}">
												 <c:set var="basketItemTotal"><mlamt:formatAmount formatPriceData ="${formatPriceData}" qty="${basketItem.qty}" frontLoadCoefficient="${frontLoadCoefficient}" value="${regularPrice}" /></c:set>
												 <c:set var="total" value="${fn:substringAfter(basketItemTotal, ' ')}" />								${PricingModel.currentCurrency}&nbsp;${total}
												  <c:set var="lcpMerchandiseTotal" value="${lcpMerchandiseTotal + total}" scope="request"/>
											 </c:when>
											 <c:otherwise>
                                        <mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${itemSubTotal}" />
											 </c:otherwise>
										</c:choose>                                        
                                    </div>
                                </c:otherwise>
                ...
                <c:if test="${GlobalCartModel.displayCartSubtotalOnItemTable or (GlobalCartModel.displayMoreItems and MoreItemCount > 0)}">
                    <div class="globalCartTotal">
                        ...
                        <c:if test="${GlobalCartModel.displayCartSubtotalOnItemTable}">
                            <c:if test="${merchandiseTotal.toBigDecimal() != basketSubTotal.toBigDecimal()}">
                                <div class="summary">
                                    ...
                                    <div class="value">
										...
										<c:choose>
											 <c:when test="${isRestrictedProduct == 'true'}">
												${PricingModel.currentCurrency}&nbsp;<ct:call object="${PricingModel}" method="getFormattedAmount" param1="${lcpMerchandiseTotal}" return="formattedAmount"/>${formattedAmount}
											 </c:when>
											 <c:otherwise>
                                        <mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${merchandiseTotal}" />
											 </c:otherwise>
										</c:choose>
                                    </div>
                                </div>
                                    ...
                                    <c:when test ="${borderFreeEnable && !selectedCountryIsUS}">
                                        <c:if test ="${discountTotal > PricingModel.zero && status.last}">
                                            <div class="discountvalue discount">
													-<mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${discountTotal}"/>
													<c:set var="disTotal"><mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${discountTotal}"/></c:set>
                                            </div>
                                        </c:if>
										<c:if test ="${status.last && empty discountTotal}">
                        					 <div class="discountvalue discount">
													-<mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${orderDiscount.amount}"/>
													<c:set var="disTotal"><mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${orderDiscount.amount}"/></c:set>
                                            </div>
                                        </c:if>
                                    </c:when>
                                    <c:otherwise>                                   
    						...
    						<c:if test="${borderFreeEnable && !selectedCountryIsUS && specialOrderDiscount > ZERO}"> 
    							<div class="summary">                                    
                                	<div class="discounttitle">
                                    	<fmt:message key="msg.borderfreee.lcp.orderDiscount" />:
                                    </div>                                
                                    <div class="value">
                                    	<div class="discountvalue discount">
                                        	<!--BorderFree - basketSpecailDiscount -->                                      
    										-<mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${specialOrderDiscount}" />
    									</div>
                                    </div>
                            	</div>     							
     						</c:if>    					
                            ...
                            <div class="summary">
                                ...
                                <div class="value">
									<c:choose>
										 <c:when test="${isRestrictedProduct == 'true'}">
											 <c:set var="formatDisTotal" value="${fn:substringAfter(disTotal, ' ')}" />											 
											 <c:set var="lcpDisSubTotal" value="${lcpMerchandiseTotal - formatDisTotal}"/>
											${PricingModel.currentCurrency}&nbsp;<ct:call object="${PricingModel}" method="getFormattedAmount" param1="${lcpDisSubTotal}" return="formattedAmount"/>${formattedAmount}
										 </c:when>
										 <c:otherwise>
				                              <mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${basketSubTotal}" />
										 </c:otherwise>
									</c:choose>
                                </div>
