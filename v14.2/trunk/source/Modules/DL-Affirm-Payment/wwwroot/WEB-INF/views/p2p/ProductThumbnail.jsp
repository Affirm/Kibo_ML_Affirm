...
<div class="ml-grid-item-image ml-thumb-item <c:if test='${onselect}'> imgdiv-carousel-selected</c:if>">
    ...
    <c:choose>
        <c:when test="${useMiniThumb}">
            <div class="ml-thumb-image-mini"><a href="${categoryItemURL}"><mlimg:prodimg src="${showSwatches and not empty ThumbnailSwatchesModel.thumbImages and not ThumbnailSwatchesModel.useDefaultThumb ? ThumbnailSwatchesModel.thumbImages[0] : ''}" imageRoot="${catalogImages.imagePath}" productID="${categoryItem.pk.asString}" imageType="mini" entityType="${categoryItem.entityType}" deferLoading="${deferLoadingThumbnail}" deferredPlaceholder="${siteImages.imagePath}global/globalgraphics/deferred_placeholder.gif" otherAttrs='alt="${categoryItem.name}" '/></a></div>
        </c:when>
        <c:otherwise>
            <div class="ml-thumb-image"><a href="${categoryItemURL}"><mlimg:prodimg src="${showSwatches and not empty ThumbnailSwatchesModel.thumbImages and not ThumbnailSwatchesModel.useDefaultThumb ? ThumbnailSwatchesModel.thumbImages[0] : ''}" imageRoot="${catalogImages.imagePath}" productID="${categoryItem.pk.asString}" imageType="thumb" entityType="${categoryItem.entityType}" deferLoading="${deferLoadingThumbnail}" deferredPlaceholder="${siteImages.imagePath}global/globalgraphics/deferred_placeholder.gif" otherAttrs='alt="${categoryItem.name}" '/></a></div>
        </c:otherwise>
    </c:choose>
    </div>
    <c:if test="${showSwatches}">
    ...
    <c:if test="${!empty categoryItem.image.thumbBadge && !useMiniThumb && showBadge}"><div class="ml-thumb-badge"><a href="<c:url value='${categoryItemURL}' />"><img src="${catalogImages.imagePath}${categoryItem.image.thumbBadge}" /></a></div></c:if>
</div>
    ...
    <div class="ml-grid-item-info ml-thumb-info">
        ...
        <c:if test="${showName}">
            <div class="ml-thumb-name ${carouselSelected} ${carouselWordwrap}">
                <a href="<c:url value='${categoryItemURL}' />">
                    <c:choose>
        ...
        <c:if test="${showFBLike}">
            <div class="ml-fbl" id='facebookLike-${categoryItem.pk.asString}' data-href="${mlsocial:generateFacebookProductURL(categoryItem, pageContext, false)}" data-show-faces="${showFacePile}">
