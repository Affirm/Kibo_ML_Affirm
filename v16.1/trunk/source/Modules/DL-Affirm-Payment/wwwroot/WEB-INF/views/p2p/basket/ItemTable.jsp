...
<c:if test="${isRestrictedProduct == 'true'}">
	<c:set var="discTotal"><mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${discountTotal}" /></c:set>
	<c:set var="formattedDiscTotal" value="${fn:substringAfter(discTotal, ' ')}" />
	<c:set var="lcpDiscSubTotal" value="${lcpMerchandiseTotal - formattedDiscTotal}" scope="request"/>
</c:if>
...
<div class="ml-basket-row ml-basket-info-container">
    ...
    <div class="ml-basket-column ml-basket-totals">
            ...
            <div class="ml-basket-column ml-basket-total-label"><fmt:message key="msg.order.shipment.merchandiseSubtotal" /></div>
				<c:choose>
					 <c:when test="${isRestrictedProduct == 'true'}">
						 <div class="ml-basket-column ml-basket-total-value">${currency}&nbsp;<ct:call object="${PricingModel}" method="getFormattedAmount" param1="${lcpMerchandiseTotal}" return="formattedAmount"/>${formattedAmount}</div>		 
					 </c:when>
					 <c:otherwise>
            <div class="ml-basket-column ml-basket-total-value"><mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${merchandiseTotal}" /></div>
					 </c:otherwise>
				</c:choose>
        </div>
                       ...
                       <c:choose>
                           <c:when test ="${borderFreeEnable && !selectedCountryIsUS}">
                               <c:if test ="${status.last && orderLevelDiscount > ZERO}">
                        					    <div class="ml-basket-column ml-basket-total-value ml-discount-value">-<mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${orderLevelDiscount}"/></div>
                               </c:if>
							   <c:if test ="${status.last && orderLevelDiscount == ZERO}">
                        					    <div class="ml-basket-column ml-basket-total-value ml-discount-value">-<mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${orderDiscount.amount}"/></div>
                               </c:if>
                           </c:when>
                           <c:otherwise>
                        				    <div class="ml-basket-column ml-basket-total-value ml-discount-value">-<mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${orderDiscount.amount}"/></div>
            ...
            <c:if test="${BasketTableModel.basket.weightSurchargeTotal > ZERO }">
                ...
                <div class="ml-basket-row ml-basket-shipping-overweight">
                    <div class="ml-basket-column ml-basket-total-label ml-overweight-label"><fmt:message key="msg.order.shipment.oversizedShipping" /></div>
							<div class="ml-basket-column ml-basket-total-value ml-overweight-value"><mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${BasketTableModel.basket.weightSurchargeTotal}" /></div>
                </div>
            </c:if>
        </div>
        ...
        <c:choose>
            <c:when test="${isRestrictedProduct == 'true'}">
                <c:set var="formattedBasketSubTotal">
                    ${currency}&nbsp;<ct:call object="${PricingModel}" method="getFormattedAmount" param1="${lcpDiscSubTotal}" return="formattedAmount"/>${formattedAmount}
                </c:set>
            </c:when>
            <c:otherwise>
                <c:set var="formattedBasketSubTotal">
        ...
        <c:if test="${showSubTotal}">
            ...
            <div class="ml-basket-row ml-basket-subtotal">
                    ...
                    <div class="ml-basket-column ml-basket-total-value">${formattedBasketSubTotal}</div>
            </div>
        </c:if>
        ...
        <c:if test="${estTaxShippingEnabled}">
            <div class="ml-est-tax-shipping">
                <tiles:insertAttribute name="esttaxshiptotals">
                    <tiles:putAttribute name="showSubTotal" value="${showSubTotal}"/>
                    <tiles:putAttribute name="formatPriceData" value="${formatPriceData}"/>
                    <tiles:putAttribute name="exchangeRate" value="${exchangeRate}"/>
                    <tiles:putAttribute name="formattedBasketSubTotal" value="${formattedBasketSubTotal}"/>
                </tiles:insertAttribute>
            </div>
        </c:if>
        ...
        <c:if test="${BasketTableModel.showYouSaved}">
            <div class="ml-basket-row ml-basket-you-saved">
                ...
                <div class="ml-basket-column ml-basket-total-value ml-basket-you-saved-value"><mlamt:formatAmount formatPriceData ="${formatPriceData}" value="${discountTotal}" /></div>
            </div>
        </c:if>
